local function download(path)
    -- Adding timestamp to URL to bypass GitHub raw cache
    local url = "https://raw.githubusercontent.com/hirokyserega-web/cc-autocraft/main/" .. path .. "?t=" .. os.epoch("utc")
    local res = http.get(url)
    if not res then return false end
    local content = res.readAll()
    res.close()
    if path:find("/") then
        local dir = path:match("(.+)/")
        if not fs.exists(dir) then fs.makeDir(dir) end
    end
    if fs.exists(path) then fs.delete(path) end
    local f = fs.open(path, "w")
    f.write(content)
    f.close()
    return true
end

local files = {
    "lib/util.lua", "lib/net.lua", "lib/itemmatch.lua", "lib/lang.lua",
    "core/storage.lua", "core/recipes.lua", "core/planner.lua", "core/dispatcher.lua",
    "core/server.lua", "worker/worker.lua", "ui/monitor.lua", "ui/widgets.lua", "config.lua", "main.lua"
}

print("Cleaning system...")
local old = {"lib", "core", "worker", "ui", "main.lua", "startup.lua"}
for _, d in ipairs(old) do if fs.exists(d) then fs.delete(d) end end

for _, f in ipairs(files) do
    print("Fetching: " .. f)
    download(f)
end

local f = fs.open("startup.lua", "w")
f.writeLine("shell.run('main.lua run')")
f.close()

print("\n!!! INSTALL COMPLETE !!!")
print("Rebooting in 3 seconds...")
os.sleep(3)
os.reboot()
